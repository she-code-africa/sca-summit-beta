...
import { MapPin, Calendar, ChevronDown, X, Check, Menu, Instagram, Linkedin, Youtube, Facebook } from "lucide-react";
...
export const BUY_TICKET_URL = "https://tix.africa/discover/she-code-africa-summit-2026/checkout?step=tickets";

function scrollToTickets(e: React.MouseEvent, router: ReturnType<typeof useRouter>, onDone?: () => void) {
  e.preventDefault();
  const doScroll = () => {
    ...
    <>
      ...
      <nav>
        <Link to="/" className="nav-logo" onClick={() => setOpen(false)}>
          <img src={scaLogo.url} alt="She Code Africa" className="nav-logo-img" />
          <span className="nav-logo-text"><br /></span>
        </Link>
        <div className="nav-links nav-links-desktop">
          <Link to="/get-involved" className="nav-link">Get Involved</Link>
          <Link to="/faqs" className="nav-link">FAQs</Link>
          <a href="#tickets" className="nav-cta" onClick={(e) => scrollToTickets(e, router)}>Get Ticket</a>
        </div>
        <button
      ...
      {open && (
        <div className="nav-mobile-panel" role="dialog" aria-modal="true">
          ...
          <Link to="/get-involved" className="nav-mobile-link" onClick={() => setOpen(false)}>Get Involved</Link>
          <Link to="/faqs" className="nav-mobile-link" onClick={() => setOpen(false)}>FAQs</Link>
          <a
            href="#tickets"
...
const COMMUNITY_BUILDER_PERKS = ["Summit access", "Networking", "Breakfast & lunch", "Hiring Room access", "Entry into the raffle draw", "Opportunity to win a laptop"];
  ...
  return (
    <div className="ticket-modal-overlay" onClick={onClose}>
      <div className="ticket-modal" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true" aria-label={`${ticket.name} ticket details`}>
        <button className="ticket-modal-close" onClick={onClose} aria-label="Close"><X size={18} /></button>
...
function TicketCard({ logo, name, price, code, groupLabel, onGetTicket }: { logo: string; name: string; price: string; code: string; groupLabel?: string; onGetTicket: () => void }) {
  const tier = code.startsWith("NXTG") || code.startsWith("NGX") ? "next-gen"
    : code.startsWith("CMBD") || code.startsWith("CBX") ? "community"
    : code.startsWith("IMLD") || code.startsWith("ILX") ? "leader"
    : "architect";
  return (
